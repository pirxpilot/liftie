...
function parseInstagram(entities) {
  ...
  entities.urls = entities.urls.filter(function(url) {
    var match = url.expanded_url.match(/http:\/\/instagram.com\/p\/([^\s\/]+)\/?/);
    if (!match) {
      return true;
    }
    entities.media.push({
      ...
      media_url_https: 'http://instagr.am/p/' + match[1] + '/media/?size=m',
      indices: url.indices
    });
    return false;
  });
